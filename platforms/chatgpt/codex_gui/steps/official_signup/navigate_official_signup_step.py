...
class NavigateOfficialSignupStep(BaseFlowStep):
    step_id = "official_signup.navigate_signup"
    stage_name = "官网注册-进入注册页"
    max_attempts = 2
    metadata = StepMetadata(
        step_id=step_id,
        stage_name=stage_name,
        intent="从官网入口进入注册表单",
        legacy_mapping="new official signup: click signup navigation target",
        expected_url_fragment="/auth/signup",
        expected_targets=("official_signup_register_button",),
    )
    ...
    def execute(self, engine, ctx):
        ...
        target = str(ctx.extra_config.get("codex_gui_official_signup_register_target") or "official_signup_register_button").strip()
        matched_url = click_and_wait_for_url(
            engine,
            driver,
            click_label="[官网注册] 进入注册页",
            target_name=target,
            fragment="/auth/signup",
